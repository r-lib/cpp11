...
Here are some Frequently Asked Questions and answers about cpp11.
...
You need to use [list initialization](https://en.cppreference.com/w/cpp/language/list_initialization) to create the object.
...
One method would be to `push_back()` the contents of the string individually.
...
#### 11. What are the types for C++ iterators?
...
#### 12. My code has `using namespace std`, why do I still have to include `std::` in the signatures of `[[cpp11::register]]` functions?

The `using namespace std` will not be included in the generated code of the function signatures, so they need to be fully qualified.
...
However if you are sure in-place modification is necessary you can use the move constructor.
