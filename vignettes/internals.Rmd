...
However it is sometimes more convenient to set your working directory to the `cpp11test` root to run only those tests.
...
We somewhat mitigate the scope of R object leaks by calling `release_existing_protections()` at the start of any wrapped cpp11 function.
