...
cpp11 is a header only library, so all source code exposed to users lives in [inst/include].
R code used to generate the exporting attributes and for `cpp11::source_cpp()` is in [R/].
Tests for _only_ the code in [R/] is in [tests/]
The rest of the code is in a separate [cpp11test/] package.
Inside [cpp11test/src] files that start with `test-` are C++ tests using the [Catch](https://testthat.r-lib.org/reference/use_catch.html) support in testthat.
In addition there are some regular R tests in [cpp11test/tests/testthat/].
...
The template parameter is the type of **value** the vector stores.
This differs from Rcpp, whose first parameter is the R API type the vector stores.

The template The class first has its declarations, then definitions further down in the file.
...
`as_cpp<>()` is a templated function that takes a SEXP and creates a C++ object from it
...
This is definitely the most complex part of the cpp11 code.
These functions use [template metaprogramming](https://en.wikipedia.org/wiki/Template_metaprogramming).
In particular the [substitution failure is not an error (SFINAE)](https://en.wikipedia.org/wiki/Substitution_failure_is_not_an_error) technique to overload the functions.
If we could use C++20 we could use [Concepts](https://en.cppreference.com/w/cpp/language/constraints) but alas.
The most common types are included in the test suite and should work, more exotic types found in real projects may cause issues.
