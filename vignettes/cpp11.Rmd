...
* Section [rcpp-intro](#rcpp-intro) teaches you how to write C++ by converting simple R functions to their C++ equivalents.
  You'll learn how C++ differs from R, and what the key scalar, vector, and matrix classes are called.

* Section [source_cpp](#sourcecpp) shows you how to use `cpp11::source_cpp()` to load a C++ file from disk in the same way you use `source()` to load a file of R code.

* Section [cpp11-classes](#cpp11-classes) discusses how to modify attributes from cpp11, and mentions some of the other important classes.

* Section [cpp11-na](#cpp11-na) teaches you how to work with R's missing values in C++.
...
* Section [cpp11-case-studies](cpp11-case-studies) shows two real case studies where cpp11 was used to get considerable performance improvements.

* Section [cpp11-package](#cpp11-package) teaches you how to add C++ code to an R package.

* Section [cpp11-more](#cpp11-more) concludes the vignette with pointers to more resources to help you learn cpp11 and C++.
...
### Using source_cpp {#source_cpp}

So far, we've used inline C++ with `cpp_function()`.
This makes presentation simpler, but for real problems, it's usually easier to use stand-alone C++ files and then source them into R using `source_cpp()`.
...
`cpp11::export` controls whether a function is exported from C++ to R; `@export` controls whether a function is exported from a package and made available to the user.

To compile the C++ code, use `source_cpp("path/to/file.cpp")`.
...
But, since C++ doesn't know that the smallest integer has this special behaviour, if you do anything to it you're likely to get an incorrect value: for example, `eval_cpp('NA_INTEGER + 1')`` gives -2147483647.
...
eval_cpp("NAN == NAN")
```
(Here I'm using `eval_cpp()` which allows you to see the result of running a single C++ expression, making it excellent for this sort of interactive experimentation.)
...
//  writable::logicals({NA_LOGICAL}),
//  writable::strings({NA_STRING})
  });
}
```

TODO: get NAs for logical and strings working for this example
...
integers findInterval2(doubles x, doubles breaks) {
  ...
  for(; it != x.end(); ++it, ++out_it) {
    doubles::const_iterator pos(std::upper_bound(breaks.begin(), breaks.end(), *it));
...
* Small note: if we want this function to be as fast as `findInterval()` in R (which uses handwritten C code), we need to compute the calls to `.begin()` and `.end()` once and save the results.
  This distracts from this example so it has been omitted.
  Making this change yields a function that's slightly faster than R's `findInterval()` function, but is about 1/10 of the code.
...
This makes vectors appropriate to use when you don't know in advance how big the output will be.
...
logicals duplicated_cpp(integers x) {
  ...
  for (int i = 0; i < n; ++i) {
    out[i] = static_cast<Rboolean>(!seen.insert(x[i]).second);
  }
  return out;
}
```
...
## Using cpp11 in a package {#cpp11-package}

The same C++ code that is used with `source_cpp()` can also be bundled into a package.
...
Re-run `cpp11:generate_exports()` whenever functions are added, removed, or have their signatures changed.
This is done automatically by the pkgbuild package.

## Learning more {#cpp11-more}
...
* [_Algorithm Libraries_](http://www.cs.helsinki.fi/u/tpkarkka/alglib/k06/), which provides a more technical, but still concise, description of important STL concepts. (Follow the links under notes.)
