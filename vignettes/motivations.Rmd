...
Copy-by-value works similarly to R, except that only copying when something is changed it _always_ copies.
...
Each of the vector classes in cpp11 has a normal *read only* version that uses copy-by-reference, and a *writable* version which uses copy-by-value.
...
Like Rcpp, cpp11 automatically generates `try / catch` guards around exported functions to prevent this and also converts C++ exceptions into normal R errors.
...
```{r, message = FALSE, results = 'asis', eval = cpp11test:::should_run_benchmarks()}
library(cpp11test)
...
cpp11 takes a more limited scope, providing only the set of vector wrappers for R vector types, coercion methods to and from C++ and the limited attributes necessary to support use in R packages.
...
| package | Rcpp compile time | cpp11 compile time | Rcpp peak memory | cpp11 peak memory |
| ---     | ---               | ---                  | ---              | ---                 |
| haven   | 16.18s            | 6.10s                | 421MB            | 200MB               |
| tidyr   | 13.40s            | 2.78s                | 361MB            |  83MB               |
...
ggplot(b_release, aes(x = len, y = min / len, color = pkg)) +
  ...
  labs(title = "log-log plot number of objects vs time to create a release a single object", x = NULL, y = NULL)
```
