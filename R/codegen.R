...
generate_exports <- function(path = ".") {
  `%>%` <- dplyr::`%>%`
  r_exports <- file.path(path, "R", "cpp11-exports.R")
  cpp_exports <- file.path(path, "src", "cpp11-exports.cpp")
  unlink(c(r_exports, cpp_exports))
  ...
  if (pkg_links_to_rcpp(path)) {
    extra_includes <- paste("#include <Rcpp.h>", "using namespace Rcpp;", sep = "\n")
  } else {
    extra_includes <-  ""
  }

  writeLines(con = cpp_exports, glue::glue('
      // Generated by cpp11: do not edit by hand
