...
cpp_register <- function(path = ".", quiet = FALSE) {
  ...
  ex_funs <- get_registered_functions(all_decorations, "cpp11::external", quiet)
  ex_fun_registration <- ""
  ex_declarations <- ""

  if (nrow(ex_funs) > 0) {
    ex_fun_registration <- glue::glue_collapse(sprintf("\n  %s", glue::glue_data(ex_funs, 
      'R_RegisterCCallable("{package}", "_{package}_ex_{name}", (DL_FUNC) {name});'
    )))
    ex_declarations <- generate_ex_declarations(ex_funs)
    generate_ex_header(path, ex_funs, package, quiet = quiet)
  }
  ...
  brio::write_lines(path = cpp_path, glue::glue('
      ...
      {init$declarations}{ex_declarations}
      extern "C" attribute_visible void R_init_{package}(DllInfo* dll){{
        R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
        R_useDynamicSymbols(dll, FALSE);{ex_fun_registration}{init$calls}
...
generate_ex_declarations <- function(funs) {
  ...
  out <- paste0("\n", glue::glue_collapse(glue::glue_data(funs,
    '{return_type} {name}({real_params}); // {basename(file)}\n\n'
  )))
  unclass(out)
}

ex_params_call <- function(params) {
  ex <- function(name="", comment="") {
    list(
      name = name,
      comment = comment
    )
  }
  if (is.list(params)) do.call(ex, params) else ex()
}

generate_ex_header <- function(path, funs, package, quiet = FALSE) {
  ex_path <- file.path(path, "inst", "include", glue::glue("{package}_api.h"))
  unlink(ex_path)
  dir.create(dirname(ex_path), recursive = TRUE, showWarnings = FALSE)

  ex_types <- c(
    file.path(path, "inst", "include", paste0(package, "_types.h")),
    file.path(path, "inst", "include", paste0(package, "_types.hpp"))
  )
  ex_types <- ex_types[file.exists(ex_types)]

  ex_types_import <- if (length(ex_types) > 0) {
    paste0("\n", glue::glue_collapse(glue::glue(
      '\n\n#include <{ex_type_file}>',
      ex_type_file = basename(ex_types)
    )))
  } else {
    ""
  }

  header_guard <- glue::glue("{package_upper}_EXTERNAL_API_H", package_upper=toupper(package))

  funs$real_params <- vcapply(funs$args, glue_collapse_data, "{type} {name}")
  funs$types_params <- vcapply(funs$args, glue_collapse_data, "{type}")
  funs$names_params <- vcapply(funs$args, glue_collapse_data, "{name}")
  funs$ex_params <- lapply(funs$params, ex_params_call)
  funs$ex_params_name <- vapply(funs$ex_params, `[[`, character(1), "name")
  funs$ex_params_comment <- vapply(funs$ex_params, `[[`, character(1), "comment")

  call_entries <- glue::glue_data(funs,
    '
    {comment}static inline {return_type} {export_name}({real_params}) {{
        static {return_type}(*p_{name})({types_params}) = NULL;
        if (p_{name} == NULL) {{
            p_{name} = ({return_type}(*)({types_params})) R_GetCCallable("{package}", "_{package}_ex_{name}");
        }}
        return p_{name}({names_params});
    }}
    ',
    export_name = ifelse(nchar(ex_params_name), ex_params_name, name),
    comment = ifelse(nchar(ex_params_comment), sprintf("// %s\n", ex_params_comment), "")
  )

  brio::write_lines(path = ex_path, glue::glue('
      // Generated by cpp11: do not edit by hand
      // clang-format off
      ...
      #include <R_ext/Rdynload.h>{ex_types_import}
  ...
  if (!quiet) {
    cli::cli_alert_success("generated file {.file {basename(ex_path)}}")
  }
}
...
check_valid_attributes <- function(decorations, file = decorations$file) {
  ...
  valid_decor <- c("cpp11::register", "cpp11::init", "cpp11::linking_to", "cpp11::external")
  bad_decor <- startsWith(decorations$decoration, "cpp11::") &
    (!decorations$decoration %in% valid_decor)
  ...
  if(any(bad_decor)) {
    ...
    msg <- glue::glue("cpp11 attributes must be one of {msg_valid}:
      {bad_lines}
      ", msg_valid = glue::glue_collapse(valid_decor, sep = ", "))
    stop(msg, call. = FALSE)
