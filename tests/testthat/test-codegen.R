...
describe("cpp_generate_bindings", {
  it("returns an invisible empty character if there are no decorations", {
    f <- tempfile()
    expect_equal(cpp_generate_bindings(f), character())

    dir.create(f)
    expect_equal(cpp_generate_bindings(f), character())

  })
  it("works with a package that exports a single c++ function", {
    pkg <- local_package()
    p <- pkg_path(pkg)
    dir.create(file.path(p, "src"))
    file.copy(test_path("single.cpp"), file.path(p, "src", "single.cpp"))
    cpp_generate_bindings(p)

    r_bindings <- file.path(p, "R", "cpp11-bindings.R")
...
extern \"C\" {
/* .Call calls */
extern SEXP _cpp11_foo();

static const R_CallMethodDef CallEntries[] = {
    {\"_cpp11_foo\", (DL_FUNC) &_cpp11_foo, 0},
    {NULL, NULL, 0}
};
}
...
extern \"C\" void R_init_cpp11(DllInfo* dll){
