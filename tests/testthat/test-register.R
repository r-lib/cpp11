test_that("pkg_links_to_rcpp works with single package in LinkingTo", {
  pkg <- local_package()
...
test_that("pkg_links_to_rcpp works with multiple packages in LinkingTo", {
  pkg <- local_package()
...
test_that("get_call_entries returns an empty string if there are no R files", {
  pkg <- local_package()
  path <- pkg_path(pkg)
  expect_equal(get_call_entries(path, get_funs(path)$name, get_package_name(path)), "")
})

test_that("get_call_entries returns an empty string if there are no .Call calls", {
  pkg <- local_package()
  path <- pkg_path(pkg)
...
test_that("get_call_entries errors for invalid packages", {
  ...
  expect_error(get_call_entries(pkg, get_funs(pkg)$name, get_package_name(pkg)), "has no 'NAMESPACE' file")
})

test_that("get_call_entries returns an empty string for packages with .Call entries and NAMESPACE files", {
...
test_that("get_call_entries works with multiple register functions", {
  pkg <- local_package()
  p <- pkg_path(pkg)
...
test_that("wrap_call works with void functions and no arguments", {
  expect_equal(
...
test_that("wrap_call works with non-void functions and no arguments", {
  expect_equal(
...
test_that("wrap_call works with void functions and some arguments", {
  expect_equal(
...
test_that("wrap_call works with non-void functions and some arguments", {
  expect_equal(
...
test_that("get_registered_functions returns an empty tibble given a non-existent file", {
  f <- tempfile()
...
test_that("get_registered_functions returns an empty tibble given an empty file", {
  f <- tempfile()
  file.create(f)
  decorations <- decor::cpp_decorations(files = f, is_attribute = TRUE)
  res <- get_registered_functions(decorations, "cpp11::register")
...
test_that("get_registered_functions works with a single registration", {
...
test_that("get_registered_functions works with multiple registrations", {
...
test_that("generate_cpp_functions returns the empty string if there are no functions", {
...
test_that("generate_cpp_functions returns the wrapped function for a single void function with no arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_cpp_functions returns the wrapped function for a single void function with no arguments and different package name", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_cpp_functions returns the wrapped function for a single function with no arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_cpp_functions returns the wrapped function for a single void function with arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_cpp_functions returns the wrapped function for a single function with arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_cpp_functions returns the wrapped functions for multiple functions with arguments", {
  funs <- tibble::tibble(
...
test_that("generate_r_functions returns the empty string if there are no functions", {
  skip_if_not_installed("glue", "1.6.2.9000")
...
test_that("generate_r_functions returns the wrapped function for a single void function with no arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_r_functions returns the wrapped function for a single void function with no arguments and use_package = TRUE", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_r_functions returns the wrapped function for a single void function with no arguments and different package name", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_r_functions returns the wrapped function for a single function with no arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_r_functions returns the wrapped function for a single function with no arguments and use_package = TRUE", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_r_functions returns the wrapped function for a single void function with arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_r_functions returns the wrapped function for a single function with arguments", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_r_functions returns the wrapped functions for multiple functions with arguments", {
  funs <- tibble::tibble(
    file = c("foo.cpp", "bar.cpp"),
    line = c(1L, 3L),
    decoration = c("cpp11", "cpp11"),
...
test_that("cpp_register returns an invisible empty character if there are no decorations", {
  f <- tempfile()
...
test_that("cpp_register works with a package that registers a single C++ function", {
  # tools::package_native_routine_registration_skeleton is not available before R 3.4
...
test_that("cpp_register can be run without messages", {
  pkg <- local_package()
  p <- pkg_path(pkg)
  dir.create(file.path(p, "src"))
  file.copy(test_path("single.cpp"), file.path(p, "src", "single.cpp"))
...
test_that("cpp_register can be run with messages", {
...
test_that("cpp_register includes pkg_types.h if included in src", {
  pkg <- local_package()
  p <- pkg_path(pkg)
  dir.create(file.path(p, "src"))
  file.copy(test_path("single.cpp"), file.path(p, "src", "single.cpp"))
...
test_that("cpp_register includes pkg_types.hpp if included in src", {
  pkg <- local_package()
  p <- pkg_path(pkg)
  dir.create(file.path(p, "src"))
  file.copy(test_path("single.cpp"), file.path(p, "src", "single.cpp"))
...
test_that("cpp_register includes pkg_types.h if included in inst/include", {
  pkg <- local_package()
  p <- pkg_path(pkg)
  dir.create(file.path(p, "src"))
  file.copy(test_path("single.cpp"), file.path(p, "src", "single.cpp"))
...
test_that("cpp_register includes pkg_types.hpp if included in inst/include", {
  pkg <- local_package()
  p <- pkg_path(pkg)
  dir.create(file.path(p, "src"))
  file.copy(test_path("single.cpp"), file.path(p, "src", "single.cpp"))
...
test_that("cpp_register does not error if no files have registered functions", {
  pkg <- local_package()
  p <- pkg_path(pkg)
  dir.create(file.path(p, "src"))
...
test_that("cpp_register accepts .cc as an alternative value for extension=", {
  pkg <- local_package()
  p <- pkg_path(pkg)
  dir.create(file.path(p, "src"))
...
test_that("generate_init_functions returns an empty list if there are no functions", {
  funs <- tibble::tibble(
    file = character(),
    line = integer(),
    decoration = character(),
...
test_that("generate_init_functions returns the declaration and call for a single init function", {
  funs <- tibble::tibble(
    file = "foo.cpp",
    line = 1L,
    decoration = "cpp11",
...
test_that("generate_init_functions returns the declaration and call for multiple init functions", {
  funs <- tibble::tibble(
    file = c("foo.cpp", "bar.cpp"),
    line = c(1L, 3L),
    decoration = c("cpp11", "cpp11"),
